...
namespace Dfc.CourseDirectory.FindACourseApi.Tests.FeatureTests
{
    public class GetTLevelListTests 
    {
        ...
        public async Task InvalidPageSizePageNumber_ReturnsBadRequest()
        {
            ...
            var pageSize = 0;
            var pageNumber = 0;
            var controller = new GetTLevelsDataController(_mediator.Object, _log.Object);
            
            // Act
            var response = await controller.TLevelsList(pageSize, pageNumber);
        ...
        public async Task ValidPageNumberPageSize_ReturnsOk()
        {
            ...
            var pageNumber = 1;
            var controller = new GetTLevelsDataController(_mediator.Object, _log.Object);
            _mediator.Setup(m => m.Send(It.IsAny<TLevelRequest>(), default)).ReturnsAsync(new TLevelResponse());
            // Act
            var response = await controller.TLevelsList(pageSize, pageNumber);
