...
{
    public class GetApprenticeshipsForProviderHandler : ISqlQueryHandler<GetApprenticeshipsForProvider, IReadOnlyCollection<Apprenticeship>>
    {
        public async Task<IReadOnlyCollection<Apprenticeship>> Execute(SqlTransaction transaction, GetApprenticeshipsForProvider query)
        {
            var sql = $@"
SELECT
    a.ApprenticeshipId,
