...
namespace Dfc.CourseDirectory.WebV2.Features.DataManagement.Apprenticeships.DeleteUpload
{
        ...
        IRequestHandler<Command, OneOf<ModelWithErrors<Command>, Success>>
    {
        ...
        public async Task<OneOf<ModelWithErrors<Command>, Success>> Handle(Command request, CancellationToken cancellationToken)
        {
            if (!request.Confirm)
            {
                ...
                {
                    new ValidationFailure(nameof(request.Confirm), "Confirm you want to delete these apprenticeships")
                });
                return new ModelWithErrors<Command>(new Command(), validationResult);
            }

            await _fileUploadProcessor.DeleteApprenticeshipUploadForProvider(_providerContextProvider.GetProviderId());
