...
namespace Dfc.CourseDirectory.WebV2.Tests.FeatureTests.DataManagement.Courses
{
    public class DownloadTests : MvcTestBase
    {
        ...
        [Fact]
        public async Task Get_DownloadNonLarsValidFile()
        {
            // Arrange
            var provider = await TestData.CreateProvider(providerName: "Test Provider");
            ...
            // Act
            var response = await HttpClient.GetAsync($"/data-upload/courses/downloadnonlars?providerId={provider.ProviderId}");

            // Assert
            response.StatusCode.Should().Be(HttpStatusCode.OK);
            response.Content.Headers.ContentType.MediaType.Should().Be("text/csv");
            response.Content.Headers.ContentDisposition.FileName.Should().Be("\"Test Provider_non_lars_courses_202104091300.csv\"");
            ...
            var rows = csvReader.GetRecords<CsvNonLarsCourseRow>();
            rows.Should().HaveCount(1);
        }
        ...
        [Fact]
        public async Task Get_WithNoNonLarsCourses_ReturnsEmptyFile()
        {
            // Arrange
            var provider = await TestData.CreateProvider(providerName: "Test Provider");
        ...
        [Fact]
        public async Task Get_WorkbasedNonLarsCourses_ReturnsCorrectResults()
        {
            // Arrange
            var provider = await TestData.CreateProvider(providerName: "Test Provider");
            await TestData.AddSectors();
            ...
            csvReader.Read();
            csvReader.ReadHeader();
            var rows = csvReader.GetRecords<CsvNonLarsCourseRow>().ToArray();
            rows.Length.Should().Be(1);
            var downloadedcourse = rows.FirstOrDefault();
        ...
        [Fact]
        public async Task Get_Online_NonLarsCourses_ReturnsCorrectResults()
        {
            // Arrange
            var provider = await TestData.CreateProvider(providerName: "Test Provider");
            await TestData.AddSectors();
