...
namespace Dfc.CourseDirectory.FindAnApprenticeship.Tests.Integration
{
    public class GetApprenticeshipsAsProviderIntegrationTests
    {
        ...
        public async Task Run_ReturnsExpectedResult()
        {
            ...
            _sqlQueryDispatcher.Setup(s => s.ExecuteQuery(It.IsAny<Dfc.CourseDirectory.Core.DataStore.Sql.Queries.GetFeChoicesByProviderUkprns>()))
                .Returns<Dfc.CourseDirectory.Core.DataStore.Sql.Queries.GetFeChoicesByProviderUkprns>(async r =>
                    JsonConvert.DeserializeObject<IEnumerable<Core.DataStore.Sql.Models.FeChoice>>(await File.ReadAllTextAsync("Integration/fechoices.json"))
