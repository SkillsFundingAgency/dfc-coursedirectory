...
namespace Dfc.CourseDirectory.Core.BinaryStorageProvider
{
    public class BlobStorageBinaryStorageProvider : IBinaryStorageProvider
    {
        ...
        {
            _log.LogInformation("Start Downloading a File from [{path}]",path);
            ...
            {
                _log.LogWarning("Failed to Download a file and Response Code [{Status}]", ex.Status);
                return false;
            }
        }
        ...
        public async Task UploadFile(string path, Stream source)
        {
            ...
            _log.LogInformation("File Upload Complete and Saved in [{path}]",path);
        }

        public async Task<IReadOnlyCollection<BlobFileInfo>> ListFiles(string path)
        {
            _log.LogInformation("Start Getting list of files from path [{path}].",path);
            if (string.IsNullOrEmpty(path))
            {
                throw new ArgumentException("Path cannot be empty.", nameof(path));
            }
            ...
            _log.LogInformation("[{count}] files retrieved from given path [{path}]", files.Count, path);
            return files;
        }
